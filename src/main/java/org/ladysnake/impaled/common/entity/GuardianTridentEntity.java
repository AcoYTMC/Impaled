    ...
    public void tick() {
        if (this.hasDealtDamage()) {
            ...
            if (this.getWorld() instanceof ServerWorld serverWorld) {
                for (int i = 0; i < 20; i++) {
                    serverWorld.spawnParticles(ParticleTypes.BUBBLE_POP, this.getX() + this.random.nextGaussian() / 10, this.getY() + this.random.nextGaussian() / 10, this.getZ() + this.random.nextGaussian() / 10, 1, this.random.nextGaussian() / 10, Math.abs(this.random.nextGaussian() / 10), this.random.nextGaussian() / 10, 0);
                }
            }
